...
| STEP seam selection/identification, repair adapters, export, and re-import evidence | Implemented in `monstertruck-io::step::continuity`, atop the direct solver API. Automatic seam discovery remains an explicit non-goal. `continuity-repair-step` exercises import, explicit adjacent shared-edge selection, a deterministic post-import edit that preserves `G0` while breaking `G1`, solve, independent certification, replacement, nonempty tessellation, export, re-import, repeated certification, and a pinned OCCT validity oracle without adding STEP dependencies to the numerical crate. |
