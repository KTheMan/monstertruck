...
The committed Phase 4 evidence cutoff for this ledger is `4c259080`. It contains
...
| Continuity replay after edits and late failure | Realistically validated for the synthetic chain | `5e22e3a3` advances and rebinds a tracking generation, replays edited geometry, rejects stale IDs, orders lexically conflicting contracts by dependency, and proves unchanged caller input after a downstream failure. | Imported modeling-session edit histories and coupled systems remain pending. |
...
| Deterministic output | Realistically validated on Windows/MSVC | Each case matches an immediate rerun and a separate-process v3 baseline verification. The raw reviewed receipt preserves geometry/report/dense/error evidence and work counters. | Cross-platform digest equivalence is not claimed. |
...
| Experimental `G4` reachability | Realistically validated as experimental | A curved-seam, multi-span, nonuniform rational quintic case certifies through order four with maximum normalized order-four residual `1.224951875e-4`. | This does not promote production `G4`; nonzero-offset and imported evidence remain pending. |
...
- **Disposition:** Fixed in `4c259080`. The public-evaluation certifier uses
  independent finite differences at endpoints, mapped knot boundaries from
  both surfaces, and span-distributed interior samples. It records absolute
  and scale-normalized finite residuals and does not describe the finite set as
  a global proof.
...
| --- | --- | --- |
| Evidence cutoff | Code and corpus evidence is committed through `4c259080`; this documentation update records the final gate reruns against that state. | Phase 4. |
| Geometry library tests | `cargo test -p monstertruck-geometry --lib` passes 175 tests. Transition, replay, resource-budget, and capability suites add 11 passing focused tests. | Phase 4. |
...
| Corpus | Full emit passes all 18 cases in 323.4 seconds. A separate process reproduces every v3 baseline digest. | Phase 4. |
| Formatting | Nightly is now installed. `cargo +nightly fmt --all -- --check` fails and proposes broad rewrites under the repository's `fn_single_line`, `where_single_line`, and `brace_style` settings. This is an open formatting gate, not the previously recorded missing-toolchain blocker. | Phase 4/repository formatting policy. |
...
| Deliverable | Status |
| --- | --- |
| Phase 4 charter and claim register | Committed as `285e1550`; open stacked pull request [`KTheMan/monstertruck#1`](https://github.com/KTheMan/monstertruck/pull/1). |
| Audit ledger | Committed through `33eff4da` in open stacked pull request [`KTheMan/monstertruck#2`](https://github.com/KTheMan/monstertruck/pull/2). |
