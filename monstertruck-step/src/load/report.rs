...
//!
//! - [`Table::entity_report`](crate::load::Table::entity_report) -- what the
...
//!   turn loss into a typed [`LoadError`](crate::load::LoadError) for the callers
...
pub enum LossCategory {
    ...
    /// [`Table::solids_via_shape_relationship`](crate::load::Table::solids_via_shape_relationship).
    Representation,
...
pub enum LossReason {
    ...
    /// [`Table::dummy`](crate::load::Table::dummy) and the hop cannot open it.
...
/// [`Table`](crate::load::Table). A default-constructed report is the honest
...
impl ShellLoadReport {
    ...
    pub fn require_lossless(&self) -> Result<(), crate::load::LoadError> {
        if self.is_lossless() {
            Ok(())
        } else {
            Err(crate::load::LoadError::PartialLoss(self.to_string()))
        }
    }
...
///
/// Carried by every [`Table`](crate::load::Table) as
/// [`Table::entity_report`](crate::load::Table::entity_report). Empty means the
...
/// in [`Table::dummy`](crate::load::Table::dummy) with their name intact, which
...
impl EntityLoadReport {
    ...
    pub fn require_empty(&self) -> Result<(), crate::load::LoadError> {
        if self.is_empty() {
            Ok(())
        } else {
            Err(crate::load::LoadError::PartialLoss(self.to_string()))
